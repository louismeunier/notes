...
This can be used to fairly easily find the inverse of a matrix, as long as the determinant is not 0.
...
A diagonalized matrix $A$ (such that $T(\vec{x}) = A\vec{x}$) is one that can be written as $[T(\vec{x})]_{\mathfrak{B}} = B[\vec{x}]_{\mathfrak{B}}$ where $B$ has the form $\begin{bmatrix}
    ...
    0 & 0 & \dots & \lambda_n
\end{bmatrix}$, where $\lambda_1, \lambda_2, \dots, \lambda_n$ are the eigenvalues of $A$. By extension, a matrix is diagonalizable if it has a eigenbasis. 
...
\subsection{Vocabulary}
    \begin{itemize}
        ...
        \item \textbf{diagonal matrix}: a matrix that has all of its entries outside of the main diagonal equal to zero, ie, $\begin{bmatrix}
            ...
            0 & b & 0\\
            0 & 0 & c\\
        \end{bmatrix}$. Sometimes the $0$'s are omitted, for the sake of brevity, such as $\begin{bmatrix}
            a & & \\
            & b & \\
            & & c\\
        \end{bmatrix}$.
