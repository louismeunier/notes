...
If $z = f(x,y)$ is implicitly defined by a function $F(x,y,z)=0$ ($\therefore F(x,y,f(x,y)) = 0$), 
...
To simplify this, let $g(h) = f(x_0+ah,y_0+bh)$, then we can say, using eqn. 83:
...
Note that this last line involves combining the result of calculating $g'(h)$ with the original definition of $g'(0)$ from eqn. 84. In summary, if $\vec{u} = \left\langle a, b \right\rangle$, then the derivative of $f(x,y)$ in the direction of $\vec{u}$ is:
...
Using this notation, we can simplify the form of the directional derivative to $D_{\vec{u}}f(x,y) = \nabla f(x,y) \cdot \vec{u}$.
