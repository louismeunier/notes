...
The derivation of this equation is very similar to that of a single-variable linearization.
...
To simplify this, let $g(h) = f(x_0+ah,y_0+bh)$, then we can say, using eqn. 84:
...
Note that this last line involves combining the result of calculating $g'(h)$ with the original definition of $g'(0)$ from eqn. 85. In summary, if $\vec{u} = \left\langle a, b \right\rangle$, then the derivative of $f(x,y)$ in the direction of $\vec{u}$ is:
...
Given a function, $f(x,y)$, a common question is to find what direction it changes the fastest, ie, what direction should $\vec{u}$ be pointing to maximize $D_{\vec{u}}f(x,y)$? This is fairly simple to calculate using eqn. 90's definition of the gradient:
...
Since $cos$ is at a max when $theta = 0$, we can thus maximize $D_{\vec{u}} f(x,y)$ by setting $\vec{u}$ in the same direction as $\nabla f(x,y)$.
% arc length
