...
\begin{enumerate}
    ...
    \item $\deriv{\hat{T}}{s} = \vec{0}$: this is only possible if the curve is a straight line, which we will assume for now is not the case. % pls verify
...
NB: the norm of $\vec{a}$ can be written as $||\vec{a}||^2 = a_T^2+a_N^2$, as per the definition of the norm of a vector. This is often a helpful result to find either of the components of $\vec{a}$ when given the other, as often $a_T$ is fairly easy to compute as it only relies on $\deriv{s}{t}$ while $a_N$ relies on knowing $\kappa$.
...
% TODO: fix drawing, add more
NB: to determine the direction of $\hat{N}$ along a planar curve, one must consider the curvature; $\hat{N}$ always points towards "inside" of curve, ie, along increasing angle of $\hat{T}$ with the horizontal.
...
% NORMAL: span of B, N (orthogonal to T)
% RECTIFYING: span of T, B (orthogonal to N)
% OSCULATING: span of T, N (orthogonal to B)
\begin{itemize}
...
For acceleration, we have: \begin{align*}
    \vec{a} &= \deriv{\vec{v}}{t} = \ddot{r} \hat{\mu}_r + \deriv{\hat{\mu}_r}{t} \dot{r} + \dot{r}\dot{\theta} \hat{\mu}_\theta + r \ddot{\theta} \hat{\mu}_\theta + r \dot{\theta} \deriv{\hat{\mu}_\theta}{t}\\
    \deriv{\hat{\mu}_r}{t} &= \dot{\theta} \hat{\mu}_\theta, \deriv{\hat{\mu}_\theta}{t} = - \dot{\theta}\hat{\mu}_r\\
    \implies \vec{a} &= \left[\ddot{r}-r\dot{\theta}^2\right]\hat{\mu}_r + \left[2 \dot{r}\dot{\theta} - r \ddot{\theta}\right]\hat{\mu}_\theta\\
    &\equiv a_r \hat{\mu}_r + a_\theta \hat{\mu}_\theta, \text{ where } a_r = \ddot{r} - r \dot{\theta}^2, a_\theta = 2 \dot{r} \dot{\theta} - r \ddot{\theta}
\end{align*}

NB: to find the radial/circumferential components of velocity/acceleration, we can simply take the dot product of the vector with the appropriate unit vector. For instance: \begin{align*}
    v_r &= \vec{v} \cdot \hat{\mu}_r; \quad v_\theta = \vec{v} \cdot \hat{\mu}_\theta\\
    a_r &= \vec{a} \cdot \hat{\mu}_r; \quad a_\theta = \vec{a} \cdot \hat{\mu}_\theta
\end{align*}

\subsubsection{Arc Length and Area of Curves with Polars}

From the previous section, we had $\vec{v}=\deriv{r}{t}\hat{\mu}_r +  r \deriv{\theta}{t} \hat{\mu}_\theta$. We can thus write:
\begin{align*}
    ||\vec{v}|| &= \sqrt{\left(\deriv{r}{t}\right)^2 + r^2 \left(\deriv{\theta}{t}\right)^2} = \deriv{s}{t}\\
    \text{d}s &= \text{d}t \sqrt{\left(\deriv{r}{t}\right)^2 + r^2 \left(\deriv{\theta}{t}\right)^2}\\
    &= \sqrt{\left(\deriv{r}{\theta}\right)^2 + r^2} \,\text{d}\theta
\end{align*}

The length of the curve between two points (well, angles, specifically) $\alpha$ and $\beta$ would thus be the sum of the changes of arc length between the points, namely \[L = s = \int ds = \int_\alpha^\beta \sqrt{r^2 + \left(\deriv{r}{\theta}\right)}\, \text{d}\theta \equiv \text{ length of curve } r = f(\theta)\]

As shown in previous sections, the area between two curves can be described as a double integral over the region described by the relevant area. In polars, we can write \[A = \int_\alpha^\beta\int_{g(\theta)}^{f(\theta)} r \, \text{d}r\text{d}\theta = \int_{\alpha}^{\beta} \frac{\left[f(\theta)\right]^2 - \left[g(\theta)\right]^2}{2} \, \text{d}\theta\]
% !!! fix area shit
\end{document}