...
The \textbf{Gamma Function} is defined \[\Gamma(n) = \int_0^\infty e^{-x} x^{n-1} \,\text{d}x,\] and is a generalization of the factorial operator, i.e., $\Gamma(n + 1) = n\Gamma(n)$. The Gamma function appears in many contexts (such as statistics), but one "helpful" (situationally) use case is the fact \[\int_0^{\pi/2} \sin^{2m-1}\theta\cos^{2n-1}\theta \,\text{d}\theta = \frac{\Gamma(n)\Gamma(m)}{2\Gamma(n+m)}; m,n > 0,\] which is fairly straightforward to prove by writing the gamma function in polar coordinates.
...
% TODO: get up to date with notes
% need to add:
% - fix formatting (dx, dy etc)
\end{document}