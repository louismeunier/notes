...
#definition("Field Extension")[
  ...
  extension of $F$ if $F$ is a subfield of $E$; we'll often denote $E\/F$.
]
...
#proposition[
  If $[E : F] < infinity$, $"Aut"(E\/F)$ cats on $E$ with finite orbits.
]
...
#proof[
  ...
  "Orb"_"Aut"(E\/F) (sigma) subset {"roots of" f "in" E},
...
$ so we wish to find the minimal $k$ such that $alpha^p^k = alpha$ for all $alpha in F$. Then, for such a $k$, the polynomial $x^p^k - x$ has at least $p^n$ roots in $F$ so it must be that $k >= n$ since any polymomial has at most its degree number of roots. But we know also that $phi^n = id$, since by the very construction of $F$ as a splitting field, $alpha^p^n = alpha$ for every $alpha in F$. So, $k = n$ i.e. $phi$ of order $n$ in $"Aut"(F\/FF_p)$. Hence, we know that $ZZ\/n ZZ subset "Aut"(F\/FF^p)$, in particular $"Aut"(F\/FF^p) >= n$.
...
#proof[
  ...
  Since $n$ prime, $(a b)$, $(1 2 dots n)$ generate $S_n$ thus $H = S_n$.
