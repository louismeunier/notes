...
\begin{theorem}[Cantor Power Set Theorem]
  Let $A$ be any set. Then there exists no surjection from $A$ onto $\mathcal{P}(A)$. 
\end{theorem}
...
\begin{lemma}[Triangle Inequality]
  ...
  \[(i)\quad|x + y|  \leq |x| + |y|; \qquad (ii) \quad|x-y| \leq |x-z| + |z-y|\]
\end{lemma}
...
\footnotetext{Note that while $x, y$ may not exist, some of the other limits may.}
\begin{remark}
...
\end{proof}
\footnotetext{
Proof sketch: take an upper bound of $x_n$. Then, show that $|x_n y_n - xy| < \epsilon$, by using triangle inequalities to show inequality to a combination of $M$, arbitrarily small values (by def of limits of $x_n, y_n$ resp,), and $|y|$. QED.
