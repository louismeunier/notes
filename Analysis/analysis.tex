...
\begin{proposition}\label{prop:infepsilon}
...
\begin{definition}[Cauchy Criterion]\label{def:cauchycriterion}
...
\begin{remark}
  AC $\iff$ BW, AC $\iff$ MCT, AC $\iff$ NIP; AC $\iff$ Cauchy Criterion $+$ Archimedian Property
\end{remark}
...
\begin{example}\label{example:recursivenessity}
  Let $(x_n)$ be a sequence defined recursively by $x_1 = 1$, $x_2 = 2$, $x_{n+1} = \frac{1}{2}(x_n + x_{n-1}), n \geq 2$. Prove that $(x_n)$ is a convergence sequence, and find its limit.
\end{example}
\begin{remark}
...
\begin{proof}(Of \cref{example:recursivenessity})\\
  ...
  \end{align*}
  which, while true, is useless. How do we find the limit?
