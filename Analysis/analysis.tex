...
\mdtheorem[style=example]{example}{\underline{Example}}[section]

\mdtheorem[style=general-thm]{theorem}{\textcolor{solarized-red}{$\rightarrowtail$ \underline{Theorem}}}[section]

\mdtheorem[style=general-thm]{proposition}{\textcolor{solarized-magenta}{$\rightarrowtail$ \underline{Proposition}}}[section]
\mdtheorem[style=general-thm]{theorem}{\textcolor{solarized-red}{$\rightarrowtail$ \underline{Theorem}}}[section]
\mdtheorem[style=general-thm]{corollary}{\textcolor{solarized-orange}{$\rightarrowtail$ \underline{Corollary}}}[section]
\mdtheorem[style=general-thm]{lemma}{\textcolor{solarized-orange}{$\rightarrowtail$ \underline{Lemma}}}[section]
\mdtheorem[style=general-thm]{axiom}{\textcolor{solarized-orange}{$\rightarrowtail$ \underline{Axiom}}}[section]
\mdtheorem[style=general-thm]{definition}{\textcolor{solarized-blue}{$\rightarrowtail$ \underline{Definition}}}[section]
...
\begin{proof}(Of (1), Concise)
  % TODO; improve previous proof. Its rather poor. This following is the same but shorter.
...
\begin{proof}
  ...
  We now show the converse. Suppose that $\forall \epsilon > 0,$ the set $\{n : \abs{x_{n} - x} < \epsilon\}$ % TODO:
...
\begin{proof}
  ...
  Suppose $x_n$ not bounded from above. Let $\alpha \in \mathbb{R},$ then, $\exists N \st x_N > \alpha$. $(x_n)$ increasing $\implies \forall n \geq N, x_n \geq x_N > \alpha \implies \lim_{n\to\infty} x_n = \infty$.
\end{proof}
\begin{proposition}
...
\begin{definition}[Cluster Point]
  Let $A \subseteq \mathbb{R}$. A point $c \in \mathbb{R}$ is called a \emph{cluster} or \emph{limit} point of $A$ if $\forall \epsilon > 0$, $\exists x \in A$, $x \neq c$, s.t. $0 < \abs{x-c} < \epsilon$.
\end{definition}
