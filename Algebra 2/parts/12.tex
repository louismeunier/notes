...
\begin{proof}
    \myuline{Linearity:} Let $\beta = \{v_1, \dots, v_n\}$ be the standard basis for $\field^n$. Fix $T_1, T_2 \in \Hom(\field^n, \field^m)$ and $\alpha \in \field$. 
    \begin{enumerate}
        \item \begin{align*}
    ...
    \end{enumerate}
    \myuline{Inverse:} We need to show that 1. $A \mapsto L_A \mapsto [L_A]$ is the identity on $M_{m \times n}(\field)$, and conversely, that 2. $T \mapsto [T] \mapsto L_{[T]}$ is the identity on $\Hom(\field^n, \field^m)$.
    \begin{enumerate}
