...
\begin{proof}
    ...
    % $\dim(V^\ast) = \dim(U^\ast) + \dim(U^\perp)$. Fix a basis $\alpha := \{v_1, \dots, v_k\}$ and complete it to a basis $\beta := \alpha \cup \{v_{k+1}, \dots, v_{n}\}$. Then, $\beta^\ast$ is a basis for $V^\ast$ and $\alpha^\ast$ is basis for $U^\ast$.
